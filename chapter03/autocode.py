...
# self.scale 高斯噪声系数
# self.weights 权重组织结构
# self.x 输入占位 [None, self.n_input]
# self.hidden 隐层 - 计算子图.
# self.reconstruction 重建结果层 - 计算子图
# self.cost 损失函数 - 计算子图 - 平方误差squared Error cost.
# self.optimizer 优化损失函数 - 计算子图 - 使用优化器优化 平方误差.
# init 初始化tf环境内 变量 - 子图
# self.sess = tf.session  获得底层C运算机会话接口
# self.sess.run(init) run执行tf init子图.
